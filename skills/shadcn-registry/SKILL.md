...
5. For one-file `registry:ui` items, omit `files` only when the published source is `<item-name>.tsx` in the item folder. List `files` explicitly for hooks, libs, blocks, pages, target paths, and multi-file items.
6. Use shadcn primitive names such as `button`, `card`, and `badge` in `registryDependencies`. Use `localRegistryDependencies` for other local registry items.
7. Run focused checks on touched files. Run `vp build` when docs, routes, registry JSON, catalog loading, or source loading changed.

...
- `--target`: required for `registry:page` and `registry:file`; optional for targeted universal `registry:item` files.
...
Use `_registry.mdx` for public metadata, usage docs, and the optional named `Preview` export. Keep `_registry.mdx` private to authoring; never list it in `files`.
...
List authored source files with paths relative to the item `_registry.mdx` file. The catalog automatically emits shadcn-facing JSON paths such as `ui/<name>.tsx`, `components/<name>.tsx`, `hooks/<name>.ts`, and `lib/<name>.ts` for the file types that install through `components.json` aliases. Use `target` for `registry:page` and `registry:file`; do not add `registry/items/**` prefixes or a separate `sourcePath` field.
